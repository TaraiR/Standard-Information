...
<table style="border-collapse:collapse;width:100%;margin:8px 0;font-size:0.9em">
  ...
  <tr><td style="padding:5px 8px;border:1px solid var(--color-border)">メモリ効率</td><td style="padding:5px 8px;border:1px solid var(--color-border)">連続領域・効率的</td><td style="padding:5px 8px;border:1px solid var(--color-border)">ポインタ分余分に使用</td></tr>
        ...
        questions: [
          ...
          {
            id: 1133,
            question: '配列と連結リストの比較として正しいものはどれか。',
            choices: [
              '配列はインデックスで任意要素にO(1)アクセスできるが、中間への挿入・削除はO(n)。連結リストは先頭・末尾への挿入はO(1)だが、任意位置へのアクセスはO(n)',
              '配列はメモリを多く消費し、連結リストはメモリを少なく消費する',
              '連結リストは連続したメモリ領域が必要で、配列はポインタでつながっている',
              '配列も連結リストも挿入・削除・アクセスはすべてO(1)で同じ',
            ],
            answer: 0,
            explanation: '配列は連続メモリのためインデックスアクセスはO(1)ですが、中間への挿入・削除は後続要素のシフトが必要でO(n)です。連結リストはポインタ更新のみで先頭への挿入はO(1)ですが、n番目の要素へのアクセスは先頭から順に辿るO(n)です。',
          },
          {
            id: 1134,
            question: '双方向連結リストの特徴として正しいものはどれか。',
            choices: [
              '各ノードが前後のノードへのポインタを持ち、前後どちらにも移動できる',
              '末尾ノードが先頭ノードを指す循環構造（循環リスト）',
              '各ノードが次のノードへのポインタのみを持つ一方向の構造（単方向連結リスト）',
              'インデックスで任意要素にO(1)アクセスできる',
            ],
            answer: 0,
            explanation: '双方向連結リストは各ノードに「前ノードへのポインタ（prev）」と「次ノードへのポインタ（next）」を持ちます。前後どちらの方向にも移動できるため、末尾からの削除がO(1)でできます。Pythonのdequeなどの内部実装に使われています。',
          },
        ],
      },
      {
...
<table style="border-collapse:collapse;width:100%;margin:8px 0;font-size:0.9em">
  ...
  <tr><td style="padding:5px 8px;border:1px solid var(--color-border)">用途</td><td style="padding:5px 8px;border:1px solid var(--color-border)">関数コール・Undo・括弧チェック</td><td style="padding:5px 8px;border:1px solid var(--color-border)">タスク待ち・BFS・印刷待ち</td></tr>
        ...
        questions: [
          ...
          {
            id: 1135,
            question: '空のスタックに対して push(1), push(2), push(3), pop(), push(4) の操作を行った後、スタックの最上部（top）の値はどれか。',
            choices: ['1', '2', '3', '4'],
            answer: 3,
            explanation: 'push(1)→[1], push(2)→[1,2], push(3)→[1,2,3], pop()→[1,2]（3が取り出される）, push(4)→[1,2,4]。最上部は最後にpushした4です。',
          },
          {
            id: 1136,
            question: '空のキューに対して enqueue(5), enqueue(3), enqueue(7) の操作後に dequeue() を1回実行した。返される値はどれか。',
            choices: ['7', '3', '5', 'エラー（キューが空）'],
            answer: 2,
            explanation: 'キューはFIFO（先入れ先出し）です。enqueue順：5→3→7。dequeue()は最も先に入った5を返します。スタック（LIFO）と混同しないよう注意。スタックなら7を返します。',
          },
        ],
      },
      {
...
<p><strong>安定ソート</strong>: 同じ値の要素の相対順序が保たれるソート（バブル・挿入・マージ）。</p>
        ...
        questions: [
          ...
          {
            id: 1137,
            question: '配列 [5, 3, 8, 1, 4] にバブルソートの1パス目を適用したとき（左の要素が大きければ交換）、何回の交換が行われるか。',
            choices: ['1回', '2回', '3回', '4回'],
            answer: 2,
            explanation: '比較順: [5,3]→5>3交換→[3,5,8,1,4](1回目), [5,8]→交換なし, [8,1]→8>1交換→[3,5,1,8,4](2回目), [8,4]→8>4交換→[3,5,1,4,8](3回目)。合計3回。1パス後は最大値8が末尾に移動します。',
          },
          {
            id: 1138,
            question: 'マージソートの説明として正しいものはどれか。',
            choices: [
              'ピボットを基準に分割・再帰し平均O(n log n)だが最悪O(n²)（クイックソート）',
              '配列を半分に分割して再帰的にソートし、2つのソート済み列をマージする。最悪でもO(n log n)保証の安定ソート',
              '隣接要素を比較・交換を繰り返すO(n²)の単純なソート（バブルソート）',
              '未ソート部分の最小値を選んで先頭と交換するO(n²)のソート（選択ソート）',
            ],
            answer: 1,
            explanation: 'マージソートは分割統治法を使い常にO(n log n)が保証される安定ソートです。大量データや安定性が必要な場面に適しています。クイックソートと異なりピボット選択の問題がないため最悪計算量がO(n²)にはなりません。ただしマージに追加のメモリO(n)が必要です。',
          },
        ],
      },
      {
...
高凝集・低結合=良い設計の原則。凝集度↑・結合度↓が保守性向上につながる。
        ...
        questions: [
          ...
          {
            id: 1115,
            question: 'スクラムの「スプリントレビュー」の目的として正しいものはどれか。',
            choices: [
              'スプリント内の作業進捗をチームで毎日確認する短い会議',
              'スプリント終了時にステークホルダーへ成果物をデモし、フィードバックをもらう',
              '次のスプリントで実施するタスクの見積もりと計画を行う',
              'スプリントの作業プロセスを振り返り改善点を話し合う',
            ],
            answer: 1,
            explanation: 'スプリントレビューはスプリント終了時にプロダクトオーナーやステークホルダーへ動作するソフトウェアをデモし、フィードバックを得る場です。選択肢1はデイリースクラム、選択肢3はスプリントプランニング、選択肢4はスプリントレトロスペクティブの説明です。',
          },
          {
            id: 1116,
            question: 'ウォーターフォールモデルとアジャイル開発の比較として正しいものはどれか。',
            choices: [
              'ウォーターフォールは変更に柔軟で、アジャイルは変更を嫌う',
              'ウォーターフォールは工程を順番に進め手戻りを前提としない。アジャイルは短い反復サイクルで変更に対応する',
              'ウォーターフォールは小規模向き、アジャイルは大規模向き',
              '両者の違いはドキュメントを作成するかどうかのみである',
            ],
            answer: 1,
            explanation: 'ウォーターフォールは要件定義→設計→実装→テストを順番に進め、前工程への手戻りを最小化します。アジャイルは1〜4週間の短いスプリントを繰り返し、要求変更に素早く対応します。どちらが優れているわけではなく、要件の安定度やチーム規模で使い分けます。',
          },
        ],
      },
      {
...
回帰テスト=修正後に既存機能が壊れていないか確認（リグレッションテスト）。
        ...
        questions: [
          ...
          {
            id: 1117,
            question: '同値分割（Equivalence Partitioning）テストの説明として正しいものはどれか。',
            choices: [
              'プログラムの内部ロジックを参照してテストケースを設計するホワイトボックステスト手法',
              '入力値を「同じ処理が行われる」グループに分割し、各グループの代表値でテストするブラックボックステスト手法',
              '境界値（最大値・最小値・境界±1）を重点的にテストする手法',
              'プログラムの全命令が最低1回実行されるようにテストケースを設計する手法',
            ],
            answer: 1,
            explanation: '同値分割は入力データを「同じ処理結果になる」グループ（同値クラス）に分け、各グループから代表値1つだけでテストします。例えば年齢入力（0〜120が有効）なら「有効クラス：50」「無効クラス：-1, 200」のように代表値を選びます。選択肢3は境界値分析の説明です。',
          },
          {
            id: 1118,
            question: 'V字モデルにおいて「結合テスト」に対応する開発工程はどれか。',
            choices: ['要件定義', '基本設計（外部設計）', '詳細設計（内部設計）', 'コーディング'],
            answer: 1,
            explanation: 'V字モデルでは左辺の各開発工程と右辺の各テスト工程が対応します。要件定義↔受入テスト、基本設計↔結合テスト、詳細設計↔単体テストが対応します。結合テストはモジュール間のインタフェースを検証するため、インタフェースを定めた基本設計が対応工程です。',
          },
        ],
      },
      {
...
ISO/IEC 25010の8特性：機能適合性・性能効率性・互換性・使用性・信頼性・セキュリティ・保守性・移植性。
        ...
        questions: [
          ...
          {
            id: 1119,
            question: 'ソフトウェアレビューの中で最も正式で体系的な手法はどれか。',
            choices: [
              'ウォークスルー（作成者が説明しながら参加者が問題を発見するインフォーマルな手法）',
              'インスペクション（モデレータが進行し、チェックリストに基づく審査で欠陥を記録・分析する正式な手法）',
              'ピアレビュー（1対1でコードを相互確認する非公式な手法）',
              '静的解析（ツールによる自動コードチェック）',
            ],
            answer: 1,
            explanation: 'インスペクションはFaganが提唱した最も正式なレビュー手法です。計画→概要説明→準備→会議（欠陥発見）→修正→フォローアップの6フェーズで進め、欠陥の記録・分析・追跡まで行います。ウォークスルーはより軽量なインフォーマルなレビューです。',
          },
          {
            id: 1120,
            question: 'ISO/IEC 25010で定義されるソフトウェア品質特性のうち「保守性（Maintainability）」の説明として正しいものはどれか。',
            choices: [
              '利用者が目標を達成するための使いやすさ（ユーザビリティ）',
              '規定された条件・時間内に正常に機能する能力（信頼性）',
              'ソフトウェアの修正・改良のしやすさ（変更・テスト・解析のしやすさを含む）',
              '外部からの不正アクセスや改ざんに対する防御能力（セキュリティ）',
            ],
            answer: 2,
            explanation: 'ISO/IEC 25010は8つの品質特性を定義します。保守性は「ソフトウェアを意図した人が修正できる度合い」で、モジュール性・再利用性・解析性・変更容易性・試験性の5つのサブ特性があります。',
          },
        ],
      },
    ],
...
WBS=作業を階層分解、ワークパッケージ=最小単位。リスクの4対応: 回避・軽減・転嫁・受容。
        ...
        questions: [
          ...
          },
          {
            ...
            choices: [
              'プロジェクトの日程を棒グラフで表し、進捗を可視化するツール（ガントチャート）',
              'プロジェクトで実施すべき全作業を成果物を基準に階層的に分解した構造',
              '各タスクの依存関係から最長経路（クリティカルパス）を求めるネットワーク図',
              'チームメンバーの担当作業と責任を一覧した表（RACI）',
            ],
            answer: 1,
            explanation: 'WBSはプロジェクトの全作業を成果物（成果・納品物）を中心に階層的に分解した構造です。「何を作るか」を明確にしてスコープを定義します。WBSを基にスケジュール（ガントチャート・PERT）やコスト見積もりを行います。',
          },
          {
            id: 1140,
            question: 'アーンドバリューマネジメント（EVM）の「コスト差異（CV）」の計算式と意味として正しいものはどれか。',
            choices: [
              'PV - AC（計画コストと実績コストの差）',
              'EV - AC（実績価値と実績コストの差）。正なら予算内、負なら予算超過',
              'EV - PV（実績価値と計画コストの差）',
              'BAC - EV（予算総額と実績価値の差）',
            ],
            answer: 1,
            explanation: 'CV（Cost Variance）= EV（Earned Value: 実際に完成した作業の予算額）− AC（Actual Cost: 実際にかかったコスト）。CV>0は予算内・コスト効率が良い、CV<0は予算超過を意味します。EV・AC・PV（Planned Value）の3つがEVMの基本指標です。',
          },
        ],
      },
      {
...
パレート図=原因を頻度順に並べ重要な原因（80:20の法則）を特定。PDCA=継続的改善サイクル。
        ...
        questions: [
          ...
          {
            id: 1141,
            question: 'QC7つ道具のうち「パレート図」の主な目的として正しいものはどれか。',
            choices: [
              '製品の寸法・重量などの計測値の分布形状を棒グラフで可視化する（ヒストグラム）',
              '不良の種類を件数の多い順に並べ累積比率を重ね、重点的に対処すべき原因上位を特定する',
              '2変数の相関関係を点で可視化する（散布図）',
              '工程の変動を時系列で監視し管理限界からの逸脱を検知する（管理図）',
            ],
            answer: 1,
            explanation: 'パレート図は不良件数を多い順の棒グラフと累積比率の折れ線グラフを重ねた図です。「重要な少数（上位20%の原因）が全体の80%の問題を引き起こす」パレートの法則に基づき、優先的に対処すべき問題を特定します。',
          },
          {
            id: 1142,
            question: 'システムの可用性（アベイラビリティ）の計算式として正しいものはどれか。',
            choices: [
              'MTBF ÷ (MTBF + MTTR)',
              'MTTR ÷ (MTBF + MTTR)',
              'MTBF × MTTR',
            ...
            answer: 0,
            explanation: '可用性 = MTBF ÷ (MTBF + MTTR)。MTBF（平均故障間隔）= 稼働時間の平均、MTTR（平均修復時間）= 修復にかかる平均時間。例えばMTBF=90時間・MTTR=10時間なら可用性=90÷100=90%。可用性を上げるにはMTBFを増やす（故障頻度を下げる）かMTTRを減らす（早く直す）。',
          },
        ],
      },
      {
...
監査の独立性: 外部監査 &gt; 内部監査。監査人は被監査業務に携わってはならない。
        ...
        questions: [
          ...
          {
            id: 1143,
            question: 'ITIL4における「変更管理（Change Management）」の目的として正しいものはどれか。',
            choices: [
              'インシデント発生時にサービスを最短時間で回復する',
              'サービスへの変更による悪影響のリスクを最小化しながら変更を確実・効率的に実施する',
              'サービス停止時の事業継続計画（BCP）を策定・維持する',
              'ユーザーからの問い合わせを一元的に受け付け管理するサービスデスク機能',
            ],
            answer: 1,
            explanation: '変更管理はサービスへの変更（インフラ・アプリ・設定など）を「承認→計画→実施→レビュー」のプロセスで管理し、予期しないサービス中断を防ぎます。変更の種類は緊急変更・標準変更・通常変更に分類されます。',
          },
          {
            id: 1144,
            question: '「システム監査」の目的として正しいものはどれか。',
            choices: [
              'ソフトウェアのバグを発見・修正するテスト活動',
              '情報システムが適切に管理・運用されているかを独立した立場（第三者・内部監査部門）が評価し、改善を勧告すること',
              'プロジェクトのコスト・スケジュールを定期的にレビューするEVM活動',
              '不正アクセスや侵入をリアルタイムで検知するセキュリティ監視',
            ],
            answer: 1,
            explanation: 'システム監査は「監査対象から独立した立場」でITシステムの有効性・効率性・安全性・信頼性を評価します。目的は問題の発見と経営者への改善勧告です。日本ではIPA（情報処理推進機構）が「システム監査基準」を定めており、基本情報試験の出題範囲です。',
          },
        ],
      },
    ],
...
// メモ化あり: F(5) で9回まで削減
        ...
        questions: [
          ...
          {
            id: 1145,
            question: '次の再帰関数 sum(4) の戻り値はどれか。\n\n関数 sum(n):\n  もし n = 0 なら: 0 を返す\n  そうでなければ: n + sum(n-1) を返す',
            choices: ['4', '6', '10', '24'],
            answer: 2,
            explanation: 'sum(4)=4+sum(3)=4+3+sum(2)=4+3+2+sum(1)=4+3+2+1+sum(0)=4+3+2+1+0=10。これは1からnまでの総和（n*(n+1)/2）を求める再帰関数です。4*5/2=10と一致します。',
          },
          {
            id: 1146,
            question: '再帰関数において「基底条件（ベースケース）」が必要な理由として正しいものはどれか。',
            choices: [
              '関数の処理速度を向上させるため',
              '無限再帰を止め、スタックオーバーフローを防いで処理を正常終了させるため',
              'メモリ使用量を削減するため',
              '並列処理を可能にするため',
            ],
            answer: 1,
            explanation: '再帰関数は自分自身を呼び出し続けるため、終了条件（基底条件）がないと無限ループになりコールスタックが溢れてスタックオーバーフローが発生します。基底条件は「これ以上再帰しない最小のケース」であり、再帰の折り返し点です。',
          },
        ],
      },
      {
...
後順（左→右→親）の訪問順: 1 → 4 → 3 → 9 → 7 → 5
        ...
        questions: [
          ...
          {
            id: 1147,
            question: '次の二分木を後順（左→右→根）でトラバーサルするとき、最初に訪問するノードはどれか。\n\n    5\n   / \\\n  3   8\n / \\\n1   4',
            choices: ['5（根）', '3（左の子）', '1（最も左の葉）', '8（右の子）'],
            answer: 2,
            explanation: '後順は「左サブツリー全体→右サブツリー全体→根」の順に再帰的に訪問します。左サブツリー（3の部分木）を後順で訪問すると「1→4→3」となります。したがって最初に訪問するノードは最も左の葉ノード1です。根5は最後に訪問します。',
          },
          {
            id: 1148,
            question: 'n個のノードを持つ完全二分木の高さ（根から最も深い葉までの辺の数）の計算量オーダーとして正しいものはどれか。',
            choices: ['O(n)', 'O(n²)', 'O(log n)', 'O(√n)'],
            answer: 2,
            explanation: '完全二分木は各レベルに2倍のノードがあるため、高さhのとき最大で2^(h+1)−1個のノードを持ちます。逆に言うとn個のノードがあるとき高さh=log₂nのオーダーです。これが二分探索・ヒープなどがO(log n)で動作する根拠です。',
          },
        ],
      },
      {
...
<table style="border-collapse:collapse;width:100%;margin:8px 0;font-size:0.9em">
  ...
  <tr><td style="padding:5px 8px;border:1px solid var(--color-border)">主な用途</td><td style="padding:5px 8px;border:1px solid var(--color-border)">最短経路・レベル順処理</td><td style="padding:5px 8px;border:1px solid var(--color-border)">迷路・連結成分・トポロジカルソート</td></tr>
        ...
        questions: [
          ...
          {
            id: 1149,
            question: '幅優先探索（BFS）と深さ優先探索（DFS）の比較で正しいものはどれか。',
            choices: [
              'DFSは最短経路（辺の数が最少）の発見に適し、BFSはスタックを使う',
              'BFSは始点から近い順に探索するため最短経路（辺の数が最少）の発見に適し、キューを使う',
              '両者とも最短経路を保証する',
              'BFSはO(V²)でDFSよりも遅い（V:頂点数）',
            ],
            answer: 1,
            explanation: 'BFS（幅優先探索）は始点から距離1のノード→距離2→距離3…と段階的に探索するためキュー（FIFO）を使い、最初に目標ノードに到達したときが最短経路（辺数が最少）です。DFSはスタック（または再帰）を使い一方向に深く進むため最短経路を保証しません。両者の計算量はO(V+E)（V:頂点数、E:辺数）で同じです。',
          },
          {
            id: 1150,
            question: 'トポロジカルソートが適用できるグラフの条件として正しいものはどれか。',
            choices: [
              '全辺が双方向の無向グラフ',
              'サイクル（閉路）のない有向グラフ（DAG: 有向非巡回グラフ）',
              '全頂点が接続された完全グラフ',
              '辺に重みが設定された重み付きグラフ',
            ],
            answer: 1,
            explanation: 'トポロジカルソートはDAG（有向非巡回グラフ）の頂点を辺の方向に沿って1列に並べる操作です。タスクの依存関係（Aが終わってからBを始める）・大学の履修前提科目・ビルドシステムのコンパイル順序などの問題に使われます。サイクルがあると順序が定まらないため適用できません。',
          },
        ],
      },
      {
...
アクセス修飾子: public=全体公開、private=クラス内のみ、protected=クラス内+子クラス。
        ...
        questions: [
          ...
          {
            id: 1127,
            question: 'オブジェクト指向の「カプセル化」の説明として正しいものはどれか。',
            choices: [
              'クラスを継承して機能を拡張すること',
              '同じメソッド名で引数や動作を変えて実装すること（オーバーロード/ポリモーフィズム）',
              'データ（属性）とそれを操作するメソッドを1つのクラスにまとめ、内部状態を外部から隠蔽すること',
              'インタフェースを実装して共通の操作を保証すること',
            ],
            answer: 2,
            explanation: 'カプセル化はデータと操作を一体化し、外部から直接アクセスできないようにする（情報隠蔽）ことです。privateで属性を隠し、publicなgetter/setterで制御したアクセスを提供します。これにより不正な状態変化を防ぎ、内部実装を変えても外部コードへの影響を最小化できます。',
          },
          {
            id: 1128,
            question: 'UMLクラス図の「集約（Aggregation）」が表す関係として正しいものはどれか。',
            choices: [
              '"is-a"関係（AはBの一種）を表す継承',
              '"uses-a"関係（AがBを使う）を表す依存',
              '"has-a"関係で部品が独立して存在できる全体・部分の関係',
              '全体が消えると部品も消える強い所有関係（コンポジション）',
            ],
            answer: 2,
            explanation: '集約（Aggregation）は「全体と部分」の関係ですが部品は独立して存在できます（例：チームと選手—チームが解散しても選手は存在する）。白いひし形で表します。選択肢4のコンポジション（黒いひし形）は全体が消えると部品も消える強い所有関係（例：家と部屋）です。',
          },
        ],
      },
      {
...
振る舞い: Template Method=処理の骨格を親が固定、Observer=状態変化を複数に通知、Strategy=アルゴリズムを実行時に差し替え、Iterator=内部構造を隠してコレクション走査。
        ...
        questions: [
          ...
          {
            id: 1129,
            question: 'Strategyパターンの説明として正しいものはどれか。',
            choices: [
              'オブジェクトに機能を動的に追加・取り外しできる構造（Decorator）',
              '処理手順の枠組みをスーパークラスに定義し、具体的な手順をサブクラスに委ねる（Template Method）',
              'アルゴリズムをクラスとして独立させ、実行時に切り替え可能にする',
              'オブジェクトの状態変化を他のオブジェクトへ自動通知する（Observer）',
            ],
            answer: 2,
            explanation: 'Strategyパターンは「行動（アルゴリズム）」をインタフェースとして抽象化し、実行時に切り替えられます。例えばソートアルゴリズム（バブル/クイック）・決済方法（クレジット/現金）など、文脈によってアルゴリズムを変えたいときに使います。if-elseの代わりにオブジェクトの交換で切り替えます。',
          },
          {
            id: 1130,
            question: 'Singletonパターンが保証することはどれか。',
            choices: [
              'クラスのインスタンスが必ず1つしか存在しないこと',
              'クラス階層を実行時に動的に切り替えられること',
              '複雑なオブジェクトの生成手順を隠蔽すること（Builder/Factory）',
              '既存インタフェースを別のインタフェースに変換すること（Adapter）',
            ],
            answer: 0,
            explanation: 'Singletonパターンはコンストラクタをprivateにしてクラス外からのインスタンス生成を禁止し、クラス自身がインスタンスを1つだけ保持・提供します。設定マネージャ・ログ出力・DBコネクションプールなど「1つしかあってはいけない」リソースの管理に使います。ただし並行処理での注意が必要です。',
          },
        ],
      },
      {
...
<p><strong>集約 vs コンポジション</strong>: 集約は部品が独立して存在できる（社員は部署がなくなっても存在する）。コンポジションは全体が消えると部品も消える（家が取り壊されると部屋も消える）。</p>
        ...
        questions: [
          ...
          {
            id: 1131,
            question: 'SOLID原則の「開放/閉鎖原則（OCP: Open/Closed Principle）」の説明として正しいものはどれか。',
            choices: [
              'クラスは1つの責務のみを持つべきである（SRP）',
              'ソフトウェアは拡張に対して開き（新機能追加OK）、修正に対して閉じている（既存コードを変えない）べきである',
              '派生クラスは基底クラスの代わりとして使えるべきである（LSP）',
              'インタフェースを通じて上位・下位モジュールの結合を疎にすべきである（DIP）',
            ],
            answer: 1,
            explanation: 'OCPは「機能を追加するとき既存コードを変更せず、新しいクラスを追加することで対応する」設計を目指します。例えば図形の面積計算に新しい図形を追加するとき、Shapeインタフェースを実装する新クラスを追加するだけで既存コードを変えずに済む設計がOCPに従っています。',
          },
          {
            id: 1132,
            question: 'SOLID原則の「インタフェース分離原則（ISP: Interface Segregation Principle）」の説明として正しいものはどれか。',
            choices: [
              '1つの大きなインタフェースより、目的別の小さなインタフェースに分割すべきである',
              'サブクラスはスーパークラスの期待する振る舞いを必ず守るべきである（LSP）',
              '具体クラスではなく抽象（インタフェース）に依存すべきである（DIP）',
              '内部実装の詳細を外部から隠蔽すべきである（カプセル化）',
            ],
            answer: 0,
            explanation: 'ISPは「クライアントが使わないメソッドへの依存を強制しない」原則です。例えば「印刷・スキャン・FAX」を1つの巨大なインタフェースにまとめると、印刷機能しか持たないクラスもスキャン・FAXメソッドの実装を強制されます。ISPに従えば3つに分離して必要なものだけ実装します。',
          },
        ],
      },
    ],
...
精度指標：正解率・適合率・再現率・F値の違いを区別できること。
        ...
        questions: [
          ...
          },
          {
            ...
            choices: [
              '訓練データ・テストデータともに精度が低い状態（学習不足）',
              '訓練データには高精度だが、未知のテストデータに対して精度が大きく低下する状態',
              '訓練データとテストデータの両方で均等に高い精度を示す理想的な状態',
              '学習率が高すぎて損失関数が収束しない状態',
            ],
            answer: 1,
            explanation: '過学習は訓練データの「ノイズや外れ値まで含めた細かいパターン」を学習しすぎた状態です。訓練精度は高いが汎化性能が低い。対策としてドロップアウト・正則化（L1/L2）・データ拡張・早期終了（Early Stopping）があります。選択肢1は未学習（Underfitting）の説明です。',
          },
          {
            id: 1122,
            question: 'ランダムフォレスト（Random Forest）の説明として正しいものはどれか。',
            choices: [
              '前のモデルの誤りを後のモデルが修正しながら順番に学習するアンサンブル手法（ブースティング）',
              '複数の決定木をランダムなデータ・特徴量のサブセットで構築し、多数決（または平均）で予測するアンサンブル手法',
              '隠れ層を深く積み重ねた多層ニューラルネットワーク',
              '教師なし学習でデータをk個のクラスタに分けるアルゴリズム',
            ],
            answer: 1,
            explanation: 'ランダムフォレストはバギング（Bootstrap Aggregating）の代表例です。複数の決定木を「ランダムなデータサンプル+ランダムな特徴量」で独立に構築し、結果を多数決・平均で統合します。単一の決定木より過学習しにくく汎化性能が高いです。選択肢1はAdaBoost・XGBoostなどブースティングの説明です。',
          },
        ],
      },
      {
...
転移学習＝学習済みモデルの知識を流用、ファインチューニング＝追加学習で特化。
        ...
        questions: [
          ...
          {
            id: 1123,
            question: 'Transformerアーキテクチャの主要な特徴として正しいものはどれか。',
            choices: [
              '前の出力が次の入力になる逐次処理構造（RNN/LSTM型）',
              'Self-Attention（自己注意機構）により文中の単語間の関係を並列処理で学習する構造',
              '局所的なパターンを検出する畳み込みフィルタを積み重ねた構造（CNN型）',
              '分類境界を最大マージンで求めるサポートベクターマシン型構造',
            ],
            answer: 1,
            explanation: 'Transformerは2017年「Attention is All You Need」論文で提案されました。Self-Attentionにより文中の各単語が他のすべての単語との関係を並列に計算でき、RNNの逐次処理の限界を克服しました。BERT・GPT・LLaMAなど現代のLLMの基盤アーキテクチャです。',
          },
          {
            id: 1124,
            question: '大規模言語モデル（LLM）の「ハルシネーション（Hallucination）」の説明として正しいものはどれか。',
            choices: [
              'モデルが質問を理解できず「わかりません」と回答する現象',
              'モデルが事実と異なる情報や存在しない事柄を、あたかも事実かのように生成する現象',
              '入力プロンプトが長すぎてモデルが処理できない現象',
              'モデルが同じ質問に毎回異なる回答を生成する現象',
            ],
            answer: 1,
            explanation: 'ハルシネーション（幻覚）はLLMが統計的に「もっともらしい」文を生成するため、事実確認なしに架空の情報を自信を持って出力する問題です。存在しない論文・法律・人物を生成することがあります。RAG（検索拡張生成）やファクトチェックの組み合わせが対策として有効です。',
          },
        ],
      },
      {
...
GDPR＝EUの個人データ保護規則。忘れられる権利・データポータビリティが特徴。
        ...
        questions: [
          ...
          {
            id: 1125,
            question: 'AIバイアス（AI Bias）への対策として適切なものはどれか。',
            choices: [
              'AIの判断を人間が監視せず完全に自律化する',
              '訓練データの多様性を確保し、公平性・偏りの観点からモデルの評価・監査を行う',
              'AIの判断プロセスをブラックボックスとして完全に非公開にする',
              '訓練データを最小化してモデルを単純化する',
            ],
            answer: 1,
            explanation: 'AIバイアスは訓練データや設計の偏りによりAIが特定のグループに対して不公平な判断をする問題です（例：採用AIが特定の性別・人種を低評価）。対策として訓練データの多様性確保、定期的な公平性監査、バイアス検出ツールの活用が重要です。',
          },
          {
            id: 1126,
            question: '説明可能AI（XAI: Explainable AI）の主な目的として正しいものはどれか。',
            choices: [
              'AIモデルの計算速度を向上させること',
              'AIが行った判断・予測の根拠を人間が理解できる形で提示すること',
              'AIをクラウド環境で効率的に運用するインフラ技術',
              '大量の訓練データを自動収集・ラベリングすること',
            ],
            answer: 1,
            explanation: 'XAIはブラックボックス化しやすいAI（特にディープラーニング）の判断根拠を人間が理解・検証できるようにする技術です。医療診断・融資審査・自動運転など高い説明責任が求められる分野で重要です。SHAP・LIMEなどの手法があります。',
          },
        ],
      },
    ],
